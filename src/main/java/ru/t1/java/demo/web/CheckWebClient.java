...
public class CheckWebClient extends BaseWebClient {
    ...
    private String resource2 = "/bsc-wire-mock/transaction/add";
    ...
    public Optional<CheckResponse> checkTransaction(Long id) {
        log.debug("Старт запроса с id {}", id);
        ResponseEntity<CheckResponse> post = null;
        try {
            CheckRequest request = CheckRequest.builder()
            ...
            post = this.post(
                    uriBuilder -> uriBuilder.path(resource2).build(),
                    request,
                    CheckResponse.class);
